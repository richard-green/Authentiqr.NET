...
namespace Authentiqr.NET.Code.EncryptionV3
{
    public class AesDecryptor
    {
        ...
        public AesDecryptor(CryptoConfig config)
        {
            ...
            if (SaltSize < 8) throw new ArgumentOutOfRangeException("AesDecryptor: SaltSize must be greater than 8");
            if (ChecksumSize < 0) throw new ArgumentOutOfRangeException("AesDecryptor: ChecksumSize must not be negative");
            if (ChecksumSize > 20) throw new ArgumentOutOfRangeException("AesDecryptor: ChecksumSize must not be greated than 20");
            if (Iterations < 0) throw new ArgumentOutOfRangeException("AesDecryptor: Iterations must not be negative");
        }
        ...
        {
            if (bytes == null || bytes.Length < SaltSize) throw new ArgumentOutOfRangeException("AesDecryptor.Decrypt: bytes cannot be null or smaller than the salt size");
